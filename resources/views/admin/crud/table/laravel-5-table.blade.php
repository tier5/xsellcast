...
<div class="padded text-right table-drop-sort">
    <div class="btn-group">
        <button data-toggle="dropdown" class="btn btn-primary dropdown-toggle">Sort by.. <span class="caret"></span></button>
        <ul class="dropdown-menu">
            @foreach($collection->eloquentTableColumns as $key => $name)
                @if(in_array($key, $collection->eloquentTableSort))
                    <li>{!! sortableUrlLink($name, array('field' => $key, 'sort'=>'asc')) !!}</li>
                @endif
            @endforeach
        </ul>
    </div>
...
<table @if($collection->eloquentTableAttributes) {!! $collection->eloquentTableAttributes !!} @else class="table table-striped" @endif>
    <thead class="@if(!$collection->isShowHeader()) hidden @endif">
    <tr>
        ...
        @foreach($collection->eloquentTableColumns as $key => $name)
            ...
            <th {!! $collection->getHiddenColumnAttributes($key) !!} data-field='{!! str_slug($name) !!}'>
                @if(in_array($key, $collection->eloquentTableSort))
